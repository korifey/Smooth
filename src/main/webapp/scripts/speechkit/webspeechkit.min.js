(function (window) {
    ...
    var scriptPath = function () {
        ...
        if (scripts && scripts.length > 0) {
            for (var i in scripts) {
                if (scripts[i].src && scripts[i].src.match(/\/webspeechkit.min.js$/)) {
                    path = scripts[i].src.replace(/(.*)\/webspeechkit.min.js$/, '$1');
                    break;
                }
            }
        }
    ...
    var WORKER_PATH = scriptPath() + '/webspeechkitWorker.min.js';
    ...
    function Recorder(stream) {
        return function (bufferSize, channelCount, onError, workerPath, outSampleRate) {
            ...
            this.start = function (cb, format) {
                ...
                worker.postMessage({
                    command: 'init',
                    config: {sampleRate: this.context.sampleRate, format: format, bufSize: this.bufferLen}
                });
                this.clear(function () {
                    recording = true;
                });
            ...
            this.exportWAV = function (cb, type) {
                ...
                worker.postMessage({command: "export" + (this.channelCount != 2 && "Mono" || "") + "WAV", type: type});
            }
...
window.webspeechkit.Dictation = function (asr_url, uuid, apikey) {
    ...
    backref.start = function (format, initCallback, errorCallback, dataCallback, infoCallback) {
        if (webspeechkit.recorderInited) {
            ...
            webspeechkit.initRecorder(function () {
                backref.onstart(format, initCallback, errorCallback, dataCallback, infoCallback);
            }, function (err) {
    ...
    backref.onstart = function (format, initCallback, errorCallback, dataCallback, infoCallback) {
        ...
        backref.recognizer = new webspeechkit.Recognizer(backref.webSocketPath(), uuid, apikey, format.mime, {
